...
namespace info {
	...
	public:
		...
		//////////////////////////////////////
		template<typename T>
		static InfoMatriceResultPairPtr perform_arrange(size_t nRows, size_t nCols, const std::vector<T> &oData,
			const strings_vector &indsNames, const strings_vector &colsNames,
			bool bComputeWeights = false,
			pcancelflag pFlag = nullptr, PBackgrounder pq = nullptr,
			MatElemFunctionType f = [](MatElemResultPtr o) {},
			...
			assert(p != nullptr);
			p->first = oMat.get_inds_result();
			p->second = oMat.get_vars_result();
			return (oRet);
		} //perform_arrange
