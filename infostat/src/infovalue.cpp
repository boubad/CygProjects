...
bool InfoValue::is_floating(void) const {
	const InfoValueType t = this->get_infovalue_type();
	return ((t == InfoValueType::floatType) || (t == InfoValueType::doubleType)
			|| (t == InfoValueType::longDoubleType));
}
...
bool InfoValue::is_text(void) const {
	const InfoValueType t = this->get_infovalue_type();
	return ((t == InfoValueType::stringType)
			|| (t == InfoValueType::wstringType));
}
...
InfoValue::InfoValue(long long ival) :
		m_val(ival) {

}
InfoValue::InfoValue(long double dval) :
		m_val(dval) {
...
InfoValue::InfoValue(float fval) :
		m_val(fval) {
	double d = (double) boost::any_cast<float>(this->m_val);
	if (std::abs(d) < EPSILON) {
		this->m_val = boost::any((float) 0);
	}
}
InfoValue::InfoValue(double dval) :
		m_val(dval) {
	double d = boost::any_cast<double>(this->m_val);
	if (std::abs(d) < EPSILON) {
		this->m_val = boost::any((double) 0);
	}
}
...
InfoValue::InfoValue(const InfoValue &other) :
		m_val(other.m_val) {
}
...
InfoValue & InfoValue::operator=(float fval) {
	if (std::abs((double) fval) < EPSILON) {
		this->m_val = boost::any((float) 0);
	} else {
